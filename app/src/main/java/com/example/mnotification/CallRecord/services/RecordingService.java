...
import com.example.mnotification.CallRecord.Activity.CallRecordActivity;
import com.example.mnotification.CallRecord.Activity.RecentCallActivity;
import com.example.mnotification.CallRecord.Activity.SettingsActivity;
...
public class RecordingService extends Service implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public static boolean isEnabled(Context context) {
        ...
        if (!Storage.permitted(context, CallRecordActivity.MUST))
            b = false;
        return b;
    }
    ...
    public void showNotificationAlarm(boolean show) {
        ...
        CallRecordActivity.showProgress(RecordingService.this, show, phone, samplesTime / sampleRate, recording);
                    ...
                    while (!Thread.currentThread().isInterrupted()) {
                        ...
                        if (stableRefresh || diff >= samples) {
                            ...
                            if (samplesTimeCount > samplesTimeUpdate) {
                                ...
                                CallRecordActivity.showProgress(RecordingService.this, true, phone, samplesTime / sampleRate, true);
                            }
                        }
                    }
                ...
                public void run() {
                    ...
                    Runnable save = new Runnable() {
                        ...
                        public void run() {
                            ...
                            CallRecordActivity.last(RecordingService.this);
                    ...
                    try {
                        ...
                        while (!Thread.currentThread().isInterrupted()) {
                            ...
                            CallRecordActivity.showProgress(RecordingService.this, true, phone, samplesTime / sampleRate, null);
                        }
                    } catch (RuntimeException e) {
                        Post(e);
                        return; // no save
    ...
    void encoding(final File in, final Uri uri, final Runnable done, final Success success) {
        ...
        final Runnable save = new Runnable() {
            ...
            public void run() {
                ...
                handle.post(new Runnable() {
                    @Override
                    public void run() {
                        CallRecordActivity.showProgress(RecordingService.this, false, phone, samplesTime / sampleRate, false);
        ...
        encoder.run(new Runnable() { // progress
            @Override
            public void run() {
                CallRecordActivity.setProgress(RecordingService.this, encoder.getProgress());
            }
    ...
    void pauseButton() {
        ...
        CallRecordActivity.showProgress(this, true, phone, samplesTime / sampleRate, thread != null);
    }
